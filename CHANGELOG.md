...
- Optional local OpenAI Responses gateway through `POST /gateway/openai/v1/responses`, gated by `gateway.enabled`, supporting non-streaming JSON responses, environment-variable API keys, local policy checks, usage response metering, and audit metadata without storing prompt or response content.
