...
可选 gateway 是本地 provider 代理，支持 OpenAI-compatible Chat Completions、OpenAI Responses 与 Anthropic Messages。它默认关闭，支持 OpenAI-compatible Chat Completions JSON/SSE 响应，以及 OpenAI Responses 和 Anthropic Messages 的非流式 JSON 响应，只从配置的环境变量读取上游 API key，并只记录 token usage 与审计元数据，不保存 request messages 或 response content。OpenAI Responses 与 Anthropic streaming 目前会明确拒绝，等 SSE usage 捕获实现后再开放。对 OpenAI Chat Completions streaming 请求，`include_stream_usage: true` 会在客户端没有显式设置 `stream_options.include_usage` 时请求兼容上游返回最终 usage chunk；如果三方中转拒绝该选项，可设为 `false`。
...
| `POST /gateway/openai/v1/responses` | 显式开启 `gateway.enabled` 后可用的 OpenAI Responses 非流式 JSON gateway |
