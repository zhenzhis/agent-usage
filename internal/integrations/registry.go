...
func Registry(opts Options) Catalog {
	...
	capabilities := []Capability{
		...
		},
		{
			...
			Category:       "gateway",
			Protocol:       "OpenAI-compatible Chat Completions HTTP JSON/SSE, OpenAI Responses HTTP JSON, and Anthropic Messages HTTP JSON proxy",
			...
			Limitations:    []string{"disabled by default", "OpenAI Chat Completions streaming usage requires upstream final usage chunks; gateway requests them by default when compatible", "OpenAI Responses and Anthropic Messages gateways support non-streaming JSON responses first", "does not persist prompt or response content"},
			NextMilestones: []string{"OpenAI Responses and Anthropic SSE usage capture", "budget-aware routing decisions", "streaming conformance fixtures for more providers"},
		},
		{
