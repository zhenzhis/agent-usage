...
func ContractVerificationReportFor(opts Options, runtime *storage.RuntimeStatus) ContractVerificationReport {
	...
	for _, path := range []string{"/api/contracts", "/api/contracts/verify", "/api/openapi.json", "/api/config/status", "/api/readiness", "/api/event-schema", "/api/events/validate", "/api/integrations/conformance", "/api/workload-events"} {
		_, ok := paths[path]
